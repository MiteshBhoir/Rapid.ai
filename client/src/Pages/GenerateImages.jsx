...
const GenerateImages = () => {
  ...
  const imageStyle = [
    'Realistic', 'Ghibli Style', 'Anime Style', 'Cartoon Style', 'Fantacy Style', 'Realistic Style', '3D style', 'Portrait Style'
  ]
  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      setLoading(true)
      const prompt = `Generate an image of the ${input} of the style ${selectedStyle}`
    ...
    <div className='h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-4 text-slate-700'>
      ...
      <form onSubmit={onSubmitHandler} className='w-full max-w-lg p-4 bg-white rounded-lg border border-gray-200 '>
        ...
        <div className='mt-3 flex gap-3 flex-wrap sm:max-w-9/11'>
          {imageStyle.map((item) => (
            <span onClick={() => setSelectedStyle(item)} key={item} className={`text-xs px-4 py-1 border rounded-full cursor-pointer ${selectedStyle === item ? 'bg-green-50 text-green-700' : 'text-gray-500 border-gray-200'}`}>{item}</span>
          ))}
        </div>
